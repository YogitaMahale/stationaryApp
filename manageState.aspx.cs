...
public partial class manageState : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            hPageTitle.InnerText = "Manage State";
        }
        ...
        if (Request.QueryString["mode"] == "u")
        {
            ...
            spnMessage.InnerText = "State Updated Successfully";
        }
        else if (Request.QueryString["mode"] == "i")
        {
            spnMessage.Visible = true;
            spnMessage.Style.Add("color", "green");
            spnMessage.InnerText = "State Inserted Successfully";
        }
    }

    private void BindCompany()
    {
        DataTable dtCompany = (new Cls_State_b().SelectAll());
        if (dtCompany != null)
        {
    ...
    protected void lnkDelete_Click(object sender, EventArgs e)
    {
        ...
        {
            Int64 CompanyId = int.Parse((item.FindControl("lblCompanyId") as Label).Text);
            bool yes = (new Cls_State_b().Delete(CompanyId));

            if (yes)
            {
                BindCompany();
                spnMessage.Style.Add("color", "green");
                spnMessage.InnerText = "State Deleted Successfully";
            }
            else
            {
                spnMessage.Style.Add("color", "red");
                spnMessage.InnerText = "State Not Deleted";
            }
        }
        else
        {
            spnMessage.Style.Add("color", "red");
            spnMessage.InnerText = "In this State city added..so you can not delete.";
        }
    }
    ...
    protected void repCompany_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if ((e.Item.ItemType == ListItemType.Item) || (e.Item.ItemType == ListItemType.AlternatingItem))
        {
            ...
            hlEdit.NavigateUrl = Page.ResolveUrl("~/addeditState.aspx?id=" + ocommon.Encrypt(DataBinder.Eval(e.Item.DataItem, "id").ToString(), true));
        }
    }

    protected void btnNewCompany_Click(object sender, EventArgs e)
    {
        Response.Redirect(Page.ResolveUrl("~/addeditState.aspx"));
    }
}